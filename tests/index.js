...
describe('glamor', () => {
  ...
  it('becomes a classname when treated as a string', () => {
    let el = (
      <div className={style({ backgroundColor: '#0f0' }) + ' wellnow'} />
    )
  ...
  it('accepts nested objects', () => {
    ...
    render(
      ...
      () => {
        ...
        expect(childStyle(node.childNodes[0]).color).toEqual(
          'rgb(255, 255, 0)'
        )
      }
    )
...
@media (min-width: 300px) {
  ...
  it('reuses common styles', () => {
    render(
      ...
      () => {
        ...
        let [ id0, id1, id2 ] = [ 0, 1, 2 ].map(i =>
          getDataAttributes(node.childNodes[0].childNodes[i])
        )
  ...
  it('can simulate pseudo classes', () => {
    ...
    simulations(true)
    render(
      <div {...hover({ backgroundColor: 'red' })} {...simulate('hover')} />,
      node,
      () => {
        simulations(false)
        expect(childStyle(node).backgroundColor).toEqual('rgb(255, 0, 0)')
      ...
      () => {
        expect(
          [ 0, 1, 2, 3 ].map(i =>
            parseInt(
              window
                .getComputedStyle(node.childNodes[0].childNodes[i])
                .color.slice(4),
              10
            )
          )
  ...
  it('can style pseudo elements', () => {
    render(<div {...firstLetter({ color: 'red' })} />, node, () => {
      expect(styleSheet.rules()[0].cssText).toEqual(
        '.css-dde0vi::first-letter, [data-css-dde0vi]::first-letter { color: red; }'
      )
    })
  }) // how do I test this?
  ...
  it('can style media queries', () => {
    ...
    render(
      <div {...media('(min-width: 300px)', style({ color: 'red' }))} />,
      node,
      () => {
        expect(childStyle(node).color).toEqual('rgb(255, 0, 0)')
        expect(
          last(styleSheet.rules())
            .cssText.replace(/\s/g, '')
            .replace('alland', '')
        ) // ie quirk
          .toEqual(
            '@media(min-width:300px){.css-1qc7hkg,[data-css-1qc7hkg]{color:red;}}'.replace(
              /\s/g,
              ''
            )
          )
  ...
  it('can target pseudo classes/elements inside media queries', () => {
    ...
    render(
      <div
        {...media('(min-width: 300px)', hover({ color: 'red' }))}
        {...simulate('hover')}
      />,
      node,
      () => {
  ...
  it('can merge rules', () => {
    ...
    render(
      <div {...merge(red, blue, hoverGn)} {...simulate('hover')} />,
      node,
      () => {
  ...
  it('select() accepts contextual selectors', () => {
    render(
      ...
      () => {
        expect(
          window.getComputedStyle(node.querySelector('.down')).color
        ).toEqual('rgb(255, 0, 0)')
        expect(
          window.getComputedStyle(node.querySelector('.notdown')).color
        ).toEqual('rgb(0, 0, 0)')
        expect(
          styleSheet
            .rules()
          ...
          '.up.css-1sk4o1x.down,.up[data-css-1sk4o1x].down{color:red;}'
        )
      }
    )
  })
  ...
  it('generates debug labels', () => {
    ...
    expect(el).toEqualJSX(
      <div data-css-1kmgssg="red.blue">
  ...
  if (isPhantom) {
    it('adds vendor prefixes', () => {
      render(
        <div {...style({ color: 'red', transition: 'width 2s' })} />,
        node,
        () => {
          expect(styleSheet.rules()[0].cssText).toEqual(
            '.css-19hf94w, [data-css-19hf94w] { color: red; transition: width 2s; -webkit-transition: width 2s; }'
          )
        }
      )
    })
    ...
    it('should be able to add fonts', () => {
      ...
      expect(styleSheet.rules()[0].cssText.replace(/\s/g, '')).toEqual(
        "@font-face { font-family: 'Open Sans'; font-style: normal; font-weight: 400; src: local(Open Sans), local(OpenSans), url(https://fonts.gstatic.com/s/opensans/v13/cJZKeOuBrn4kERxqtaUH3ZBw1xU1rKptJj_0jans920.woff2) format(woff2); }".replace(
          /\s/g,
          ''
        )
      )
  ...
  it('can generate css from rules', () => {
    ...
    expect(cssFor(red, merged)).toEqual(
      '.css-1ezp9xe,[data-css-1ezp9xe]{color:red;}.css-11t3bx0,[data-css-11t3bx0]{color:red;}.css-11t3bx0:hover,[data-css-11t3bx0]:hover{color:blue;}'
    )
  })
  ...
  it('can generate html attributes from rules', () => {
    ...
    expect(attribsFor(red, merged)).toEqual(
      'data-css-1ezp9xe="" data-css-11t3bx0=""'
    )
    cssLabels(true)
  })
  ...
  it('checks for a cache miss', () => {
    ...
    expect(() => merge(myObscureStyle)).toThrow(
...
describe('clean', () => {
  ...
  it('handles css variables', () => {
    expect(createMarkupForStyles({ '--myVar': 'value' })).toEqual(
      '--myVar:value;'
    )
    expect(createMarkupForStyles({ '--my-var': 'value' })).toEqual(
      '--my-var:value;'
    )
    expect(createMarkupForStyles({ '--myVar': 'value', fontSize: 20 })).toEqual(
      '--myVar:value;font-size:20px;'
    )
  })
})
...
describe('falsy values', () => {
  ...
  [ null, false, undefined ].forEach(falsy => {
    ...
    check(style, [ { fontSize: 10, color: falsy } ], [ { fontSize: 10 } ])
    check(select, [ '', { fontSize: 10, color: falsy } ], [ '', { fontSize: 10 } ])
    check(parent, [ '', { fontSize: 10, color: falsy } ], [ '', { fontSize: 10 } ])
    check(merge, [ { fontSize: 10 }, falsy ], [ { fontSize: 10 } ])
    check(
      media,
      [ '()', { fontSize: 10, color: falsy } ],
      [ '()', { fontSize: 10 } ]
    )
    check(simulate, [ 'hover', falsy ], [ 'hover' ])
    check(
      css.fontFace,
  ...
  it('can rehydrate from serialized css/cache data', () => {
    ...
    if (styleTag.styleSheet) {
      ...
      styleTag.appendChild(
        document.createTextNode('[data-css-1ezp9xe]{ color:red; }')
      )
    }
...
__webpack_nonce__ = 'test-nonce' //eslint-disable-line
...
describe('StyleSheet', () => {
  ...
  it('can initialize', () => {
    expect(
      [ ...document.styleSheets ].filter(s => s.ownerNode === sheet.tags[0])
        .length
    ).toEqual(1)
  ...
  it('can add css', () => {
    ...
    expect(window.getComputedStyle(node.childNodes[0]).color).toEqual(
      'rgb(255, 0, 0)'
    )
    document.body.removeChild(node)
  })
  ...
  it('assigns a nonce', () => {
    ...
    expect(style.getAttribute('nonce')).toEqual('test-nonce')
  })
...
describe('react', () => {
  ...
  it('propMerge will merge styles with react props', () => {
    ...
    let SpecificContainer = props => (
      <DefaultContainer {...specificStyle} {...props} />
    )
...
describe('jsxstyle', () => {
  ...
  it('exposes a View component', () => {
    render(
      ...
      node,
      () => {
        expect(styleSheet.inserted).toEqual({
          '1v0yc9d': true,
          '1v1ok9d': true,
          o6qtyr: true,
          r1q63t: true
