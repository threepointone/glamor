...
export const conversions = {
  ...
  MediaQuery(node, ctx) {
    if (node.prefix) {
      return `${node.prefix} ${node.type} ${node.exprs
        .map((x) => convert(x, ctx))
        .join(" ")}`; // todo - bug - "and"
    } else {
  ...
  RuleSet(node, ctx) {
    ...
    let x = {
      [selector]: Object.assign(
        {},
        ...node.declarations.map((x) => convert(x, ctx))
      ),
  ...
  Selector(node, ctx) {
    return `${convert(node.left, ctx)}${node.combinator}${convert(
      node.right,
      ctx
    )}`;
  },
  SimpleSelector(node, ctx) {
    let ret = `${
      node.all ? "*" : node.element !== "*" ? node.element : ""
    }${node.qualifiers.map((x) => convert(x, ctx)).join("")}`;
    return ret;
  },
  Contextual() {
    return "&";
  ...
  AttributeSelector(node, ctx) {
    return `[${node.attribute}${
      node.operator ? node.operator + node.value : ""
    }]`;
  },
  Function() {},
  ...
  Expression(node, ctx) {
    return (
      convert(node.left, ctx) +
      (node.operator || " ") +
      convert(node.right, ctx)
    );
  },
  Stub(node, ctx) {
